...
function random_id()
{
$sql="select user_id from game_pro_user where game_id=11";
$res=mysql_query($sql);
...
for($j=0;$j<$arr_id_no;$j++)
{
	...
	$sql2="UPDATE `game_pro_user` SET `group`='$group_name' WHERE (`user_id`='$arr_ran_id[$j]')";
	//$sql3="INSERT INTO groups_game (group_count, group_name) VALUE ('25','$group_name')";
...
for($h=1;$h<=5;$h++)
{
	$sql3="INSERT INTO game_groups (cards_no, game_group) VALUE (25,'a$h')";
		mysql_query($sql3);
}
}
...
function display_cards($g,$dis,$user_id)
{
	...
	for($k=count($a1);$k<=$no_of_cards;$k++)		//at the first time should be $cards_no=25
	{
		$a1[$k]=-$k;
		//echo"<br>".$a1[$k];
	}
	/*foreach ($a1 as $key)
{
	...
	for($i=0;$i<$no_of_cards;$i++)		//at the first time should be $cards_no=25
	{
	$group1[$i]=0;	
	
	}
/*while($check_group<25 )
{
	$r=rand_no(25);
	...
	$i=0;
while($i<=$no_of_cards )		//at the first time should be $cards_no=25
{
	$r=rand_no($no_of_cards);
	//echo $r."<br>";
	if(!in_array($r,$locations) )
	{ 
		//echo $r."<br>";
...
for ($j=0;$j<$no_of_cards;$j++)
	{ 
		...
		if($group1[$j]==$user_id)
		{
			//echo"ffffffffff";
			//echo"<br>".$group1[$j];
	echo '<input type="image" class="gameButton" src="images/girl-Button.jpg" width="56" height="55" disabled  value="'.base64_encode($group1[$j]).'" name="'.$group1[$j].'" onclick="player_click(this)"/>&nbsp&nbsp';

		}
		else
		{
			//echo"yes";
	echo '<input type="image" class="gameButton" src="images/Games-Button.jpg" width="56" height="55" '.$dis.'  value="'.base64_encode($group1[$j]).'" name="'.$group1[$j].'" onclick="player_click(this)"/>&nbsp&nbsp';
...
if (xmlhttp.readyState==4 && xmlhttp.status==200)
  ...
  {
	  //refresh the page
	  window.location='game.php?current_id=<?= $_SESSION['User_ID']; ?>';
	  document.getElementById("game_result").innerHTML=xmlhttp.responseText;
 	  btn.type='hidden';
  }
  else
	  //ref
  window.location='game.php?current_id=<?= $current_id; ?>';
...
<script language="JavaScript">
  ...
  function CountBack(myDiv, secs)
  {
    ...
    else
    {
      ...
      player_click("non");
     // window.location='game.php?current_id=<?= $current_id; ?>&time_at_click=<?= strftime('%c') ?>';
